...
WXData=FillMissWX(declat=myflowgage$declat, declon=myflowgage$declon,
                  ...
                  date_max="2023-02-22",targElev=myflowgage$elev,
                  method = "IDW",alfa=2)
...
source("https://raw.githubusercontent.com/tyler-26-smith/BSE5304Labs/main/R/TMWBFunc.R")

# Lets make one out of our Temperature Index Snow Model
#

source("https://raw.githubusercontent.com/tyler-26-smith/BSE5304Labs/main/R/TISnowFunc.R")

SNO_df=TISnow(TMWB, SFTmp=2, bmlt6=1, bmlt12=0, Tmlt=1)
TMWB$SNO=SNO_df$SNO
...
####################
detach(TMWB)
source("https://raw.githubusercontent.com/tyler-26-smith/BSE5304Labs/main/R/TMWBModel.R")
...
legend("topright", c("Observed", "Storage 45", "Storage 260"), col = c("black", "red", "blue"),
       lty = 1:2, cex = 0.8)
title(main="CN (TMWB Uncalibrated Stoarage)")
...
y <- optimize(f, c(50,1000), tol = 0.0001,maximum = TRUE)$maximum
Sest=y
plot(dP,Qmm)
points(dP,dP^2/(dP+Sest),col="red")
legend("topright", c("Observed", "Storage 501"), col = c("black", "red"),
       lty = 1:2, cex = 0.8)
title(main="CN (TMWB Calibrated Stoarage)")
########
detach(BasinTMWB_JO)

source("https://raw.githubusercontent.com/tyler-26-smith/BSE5304Labs/main/R/CNModel.R")

NSE(TMWB$Qmm, TMWB$Qpred)
NSE(dP, Qmm)
    
########### CNII Hydro Model########
plot(CNmodeldf$date,CNmodeldf$Qmm,col="black",ylab ="Qmm(mm)",xlab="date",type="l")
lines(CNmodeldf$date,CNmodeldf$Qpred,col="blue",type="l", 
      xlab = "", ylab = "")
legend("topright", c("Qmm(mm)", "Qpred(mm)"), col = c("black", "blue"),
       lty = 1:2, cex = 0.8)
title(main="CNII Model Hydrograph")

############ CNII CN#######
attach(CNmodeldf)
plot(dP,Qmm)
points(dP,dP^2/(dP+Sest),col="red")
legend("topright", c("Observed", "Storage 84.67"), col = c("black", "red"),
       lty = 1:2, cex = 0.8)
title(main="CNII")
NSE(dP,Qmm)
NSE(Qmm,dP^2/(dP+84.67))
detach(CNmodeldf)
